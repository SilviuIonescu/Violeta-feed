...
function cardInfo(card){
 ...
 return{
  ...
  hook:card.querySelector('.hook').textContent,
  image:getComputedStyle(hero).backgroundImage,
  position:getComputedStyle(hero).backgroundPosition
 };
}
function bind(){
...
function openCard(i){
 ...
 const card=feed.querySelectorAll('.story-card')[i];
 const c=cardInfo(card);
 inner.innerHTML=`<div class="modal-hero" id="articleHero"><div class="modal-gradient"></div><div class="article-reason">${c.reason}</div><h1 class="modal-title">${c.title}</h1></div><div class="modal-content"><div class="category">${c.category}</div><div class="modal-hook">${c.hook}</div><div class="summary-label">AI SUMMARY</div><p class="modal-summary">This story was selected because it matches Violeta’s interests, current mood and location. The key idea is simple: ${c.hook}</p><div class="summary-label">THE 2-MINUTE VERSION</div><div class="modal-sections"><section class="modal-section"><h3>What matters</h3><p>${c.title} is less about one isolated fact and more about the change behind it.</p></section><section class="modal-section"><h3>Why it was selected</h3><p>It is timely, easy to understand and likely to create a useful conversation rather than more noise.</p></section><section class="modal-section"><h3>What to notice next</h3><p>Look for what changes in everyday life—not only in headlines.</p></section></div><div class="modal-actions"><button class="modal-like">♡ <span>Like</span></button><button class="modal-share">↗ <span>Share</span></button></div><a class="source-link" href="https://news.google.com" target="_blank" rel="noopener">Open original article ↗<small>Public source</small></a><div class="summary-label">AI REACTIONS</div><div class="modal-comments"><div class="mini-comment"><div class="avatar">P</div><div><strong>Practical AI</strong><span>Useful when it helps you decide what matters next.</span></div></div><div class="mini-comment"><div class="avatar">S</div><div><strong>Skeptic AI</strong><span>The headline is only the beginning; context still matters.</span></div></div><div class="mini-comment"><div class="avatar">F</div><div><strong>Friend AI</strong><span>This one sounds worth discussing over coffee.</span></div></div></div></div>`;
 const articleHero=inner.querySelector('#articleHero');
 articleHero.style.backgroundImage=c.image;
 articleHero.style.backgroundPosition=c.position||'center center';
...
function closeCard(){modal.classList.remove('open');modal.setAttribute('aria-hidden','true');document.body.style.overflow='';requestAnimationFrame(()=>window.scrollTo(0,feedScrollY))}
